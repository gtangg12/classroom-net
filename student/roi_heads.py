...
class RoIHeads(nn.Module):
    ...
    def postprocess_detections(
        ...
        pred_depths_list = depth_prediction.split(boxes_per_image, 0)

        all_boxes = []
        all_scores = []
        all_labels = []
        all_depths = []
        for boxes, scores, depths, image_shape in zip(pred_boxes_list, pred_scores_list, pred_depths_list, image_shapes):
            ...
            boxes, scores, labels, depths = boxes[inds], scores[inds], labels[inds], depths[inds]

            # remove empty boxes
            keep = box_ops.remove_small_boxes(boxes, min_size=1e-2)
            boxes, scores, labels, depths = boxes[keep], scores[keep], labels[keep], depths[keep]
        ...
        return all_boxes, all_scores, all_labels, all_depths
    ...
    def forward(
        ...
        class_logits, box_regression, depth_prediction = self.box_predictor(box_features)
